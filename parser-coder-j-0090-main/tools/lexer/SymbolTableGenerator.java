...
public class SymbolTableGenerator extends FileGeneratorTool {
  ...
  private String symbolMethod() {
    return String.join("", Arrays.asList(
        this.formatLine(String.format(
            "public static Symbol symbol(String lexeme, %s kind) {",
            TokenKindGenerator.GENERATED_CLASS_NAME), 1, 1),
        this.formatLine("Symbol s = symbols.get(lexeme);", 2, 2),
        this.formatLine("if (s == null) {", 2, 1),
        this.formatLine(String.format(
            "if (kind == %s.BogusToken) {", TokenKindGenerator.GENERATED_CLASS_NAME), 3, 1),
    ...
    try (FileWriter writer = new FileWriter(this.getGeneratedFilePath().toFile())) {
      ...
      while (this.hasNext()) {
        ...
        writer.write(this.formatLine(String.format(
            "symbols.put(\"%s\", new Symbol(\"%s\", %s.%s));",
            lineParts[1], lineParts[1], TokenKindGenerator.GENERATED_CLASS_NAME, lineParts[0]), 2, 1));
      }
